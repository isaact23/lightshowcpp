...
void NeoPixel::fatal(std::string fmt) {
...
unsigned int NeoPixel::mem_phys_to_virt(uint32_t phys){
    ...
    fatal("Failed to reverse map phys\n");
...
void* NeoPixel::map_peripheral(uint32_t base, uint32_t len){
    ...
    if (vaddr == MAP_FAILED) {
        ...
        fatal("Failed to map peripheral\n");
    }
    close(fd);
...
void NeoPixel::initHardware(){
    ...
    if (virtbase == MAP_FAILED) {
        ...
        fatal("Failed to mmap physical pages\n");
    }
    ...
    if (page_map == 0) {
        ...
        fatal("Failed to malloc page_map\n");
    }
